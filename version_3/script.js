...
function setupReveal() {
  ...
  document.querySelectorAll(".reveal, .feature-card").forEach((element, index) => {
...
function setupProgressText() {
  const element = document.querySelector("[data-progress-text]");
  ...
  function update() {
    ...
    const progress = Math.min(1, Math.max(0, (viewport * 0.82 - rect.top) / (rect.height + viewport * 0.34)));
    const visibleCount = Math.floor(progress * letters.length);
    letters.forEach((letter, index) => letter.classList.toggle("visible", index <= visibleCount));
  }
